public static List<List<Integer>> fourSum(int[] a, int target) {
        ...
        Arrays.sort(a);
        Four_Sum_Trim(a, target, 4,0, result, new ArrayList<>());
        return result;

    }

     public static void Four_Sum_Trim(int[] a, int target ,int k,int start, List<List<Integer>> result, List<Integer> path) {
        ...
        if (k == 2) {                        // 2 Sum
            ...
            for (int i = start; i < a.length - k + 1; i++) {
                ...
                path.add(a[i]);
                Four_Sum_Trim(a, target - a[i], k - 1, i + 1, result, path);
